...
** DONE Support entering grades for everyone in course
...
** DONE Support escaping in submenus back to main menu.
Easiest, and expected, thing here is to trap KeyboardInterrupt.

** TODO [#A] Fix missing values behavior in calculator_helpers          :bug:

Lots of functions call remove_none_and_nan.  But:
  1) they don't remove the corresponding weights, if they are
     computing a weighted average.  What to do here?
  2) they don't remove 'I' grades from letter grades.
